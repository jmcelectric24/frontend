...
const LTDataPage = async ({ params }: { params: { slug: string[] } }) => {
  ...
  const ltDataRef = collection(db, "ht_data");
  const filteredQuery = query(ltDataRef, where("month", "==", prevMonth));
  const querySnapshot = await getDocs(filteredQuery);
  const data = querySnapshot.docs[0].data();

  const downloadData: any = []
  const downloadRef = collection(db, `ht_download_data_${data_filter}`);
  ...
  const options = [
    {
      label: "Total Live HT Connections",
      value: "main_table",
    },
    {
      label: "HT Connections with MD-CD > 20%",
      value: "connections_with_mdsl_gt_20_table",
    },
    {
      label: "HT Connections with CD-MD > 10%",
      value: "connections_with_slmd_gt_10_table",
    },
    {
      label: "HT Connections with power-factor less than 0.9",
      value: "connections_with_pf_lt_8_tenths_table",
    },
    {
      label: "HT Connections with zero Energy Consumption",
      value: "connections_with_zero_consumptions_table",
    },
    {
      label: "HT Connections with Energy Variations of +5%",
