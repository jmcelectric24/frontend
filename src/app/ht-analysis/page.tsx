...
const LTAnalysisPage = async () => {
  ...
  const homepageRef = collection(db, 'ht_analysis');
  ...
  const loadAnalysisTableData: TableRow[] = [
    ...
    { "Connections With MD-CD > 20%": data["load_analysis"]["connections_with_mdsl_gt_20"] },
    { "Connections with CD-MD > 10%": data["load_analysis"]["connections_with_slmd_gt_10"] },
  ]

  const pfAnalysisTableHeading = ["PF Analysis"]
  const pfAnalysisTableData: TableRow[] = [
    { "Connections with pf < 0.9": data["pf_analysis"]["connections_with_pf_lt_8_tenths"] },
    { "Total Surcharge": data["pf_analysis"]["total_pf_surcharge"] },
  ]
